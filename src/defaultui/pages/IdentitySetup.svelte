            ...
            await new Promise<void>((resolve)=>{
                setTimeout(async ()=>{
                    try {
                        ...
                        if (!this.isDIDOnChain() && !this.isDIDBeingPublished()) {
                            ...
                            let interval = setInterval(() => {
                                if(progress >= 0.90) {
                                    ...
                                    progress += 0.01;
                                    componentChanged();
                                    storage.set('progressDate', new Date());
                        ...
                        if (!this.isDIDOnChain() && this.isDIDBeingPublished()) {
                            const progressDate = await storage.get('progressDate', null);
                            const storageProgress = parseFloat(await storage.get('progress', null));
...
<content>
    {#if !componentWatcher("wasTemporaryIdentityCreationStarted")}
        <div class="swiper-container" class:dark-mode={theme.darkMode}>
            <Swiper
